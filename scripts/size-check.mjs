...
const limits = Object.freeze({ core: 60_000, ru: 8_000, en: 6_000, textsieve: 1_000 });
...
for (const [name, limit] of Object.entries(limits)) {
  ...
  const packageName = name === "textsieve" ? name : `@textsieve/${name}`;
  console.log(`${packageName} runtime JS: ${bytes}/${limit} bytes`);
}
